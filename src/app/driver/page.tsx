...
import { Navigation, ShieldCheck, DollarSign, Star, FileText, CheckCircle, ToggleLeft, ToggleRight, Play, X, User } from "lucide-react";
    ...
    <div className="w-full min-h-screen bg-gray-950 grid-bg py-12 px-6 md:px-12 text-left">
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-white flex items-center gap-2">
              ...
              {driver?.documentVerified ? (
                ...
                <span className="text-xs bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 px-2.5 py-0.5 rounded-full font-bold font-mono">
        ...
        {driver?.status === "online" && !driver?.activeOrderId && newOrders.length > 0 && (
          <div className="w-full animate-pulse border border-yellow-500/30 bg-yellow-500/5 p-6 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <span className="text-[10px] bg-yellow-500 text-black px-2 py-0.5 rounded font-extrabold font-mono tracking-wider">NEW DISPATCH SIGNALED</span>
            ...
            <div className="flex gap-2 w-full md:w-auto shrink-0">
              <button
                ...
                className="flex-1 md:flex-none px-6 py-2.5 bg-primary hover:bg-primary-hover text-white text-xs font-semibold rounded-lg transition-all cursor-pointer"
              >
                Accept Order (Fare: ₹{newOrders[0].price.toFixed(0)})
              </button>
              <button
                onClick={() => handleRejectOrder(newOrders[0].id)}
                className="px-4 py-2.5 bg-white/5 border border-white/10 hover:bg-white/10 text-white text-xs font-semibold rounded-lg transition-all cursor-pointer"
              >
                Reject
              </button>
            </div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            {activeOrder ? (
              <div className="glass-card p-6 rounded-3xl border border-white/10 shadow-2xl flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-primary uppercase">Current Active Route</span>
                  <span className="text-[10px] font-mono text-gray-400">ID: #{activeOrder.id.slice(0, 8)}</span>
                </div>

                <div className="bg-white/5 p-4 rounded-xl border border-white/5 text-xs flex flex-col gap-2">
                  <p className="text-gray-400 font-bold">PICKUP FROM:</p>
                  <p className="text-white truncate mb-2">{activeOrder.pickup.address}</p>
                  <p className="text-gray-400 font-bold">DELIVER TO:</p>
                  <p className="text-white truncate">{activeOrder.drop.address}</p>
                  {activeOrder.packageNotes && (
                    <p className="text-yellow-500 mt-1 italic">Notes: {activeOrder.packageNotes}</p>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center text-xs">
                  <div>
                    ...
                    <p className="text-white font-extrabold capitalize text-base mt-0.5">{activeOrder.status.replace("_", " ")}</p>
                  </div>
                  <div>
                    <span className="text-gray-400 font-semibold uppercase">EARNING FARE</span>
                    <p className="text-white font-extrabold text-base mt-0.5">₹{activeOrder.price.toFixed(2)}</p>
                  </div>
                </div>

                <button
                  onClick={advanceWorkflow}
                  className="w-full py-3 rounded-xl bg-primary hover:bg-primary-hover text-white text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
              ...
              <div className="glass-card p-6 rounded-3xl border border-white/5 text-center flex flex-col items-center gap-3 py-10">
                <Navigation className="w-8 h-8 text-gray-500 rotate-45" />
                <h4 className="text-sm font-semibold text-white">No Active Job</h4>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="glass-card p-5 rounded-2xl border border-white/5">
                ...
                <span className="text-[10px] text-gray-400 font-bold uppercase block">Total Earnings</span>
              ...
              <div className="glass-card p-5 rounded-2xl border border-white/5">
                ...
                <span className="text-[10px] text-gray-400 font-bold uppercase block">Driver Rating</span>
                <span className="text-2xl font-extrabold text-white mt-1 block">
            ...
            <div className="glass-card p-6 rounded-3xl border border-white/5 flex flex-col gap-4 text-left">
              ...
              {docsSubmitted ? (
                <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl text-xs flex flex-col gap-1.5 font-semibold">
                  <p>✓ Documents submitted for verify!</p>
                ...
                <form onSubmit={handleDocSubmit} className="flex flex-col gap-4 text-xs">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className="px-3 py-2.5 rounded-xl glass-input text-white text-xs focus:outline-none"
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-white/5 border border-white/10 hover:bg-white/10 rounded-xl font-bold text-white transition-all cursor-pointer"
                  >
